...
class SelectablePaymentNotifier extends ValueNotifier<dynamic> {
  final _logger = Logger();
  final PaymentRepository _paymentRepository;
  final BuildContext _context;
  late final PublishSubject<SelectablePaymentEvent> _eventSubject;

  SelectablePaymentNotifier(this._paymentRepository, this._context)
      : super(dynamic);

  void init(PublishSubject<SelectablePaymentEvent> eventSubject) {
    ...
    _eventSubject.stream
      ...
      _logger.e("Error handling selectable payment event", error, stackTrace);
    });
  }

  /// Handles [SelectablePaymentEvent]s
  _handleEvent(SelectablePaymentEvent event) {
  ...
  void _makePayment({required String email, required double amount}) {
    _paymentRepository
        .selectablePayment(email: email, amount: amount, context: _context)
        .then((response) {
